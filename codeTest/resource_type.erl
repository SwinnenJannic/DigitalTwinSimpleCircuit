...
-export([create/2]).
-export([get_initial_state/3, get_initial_state/2, get_connections_list/2, get_locations_list/2]).
...
get_initial_state(ResTyp_Pid, ResInst_Pid) ->
	msg:get(ResTyp_Pid, initial_state, [ResInst_Pid]).

get_initial_state(ResTyp_Pid, ResInst_Pid, TypeOptions) ->
	%io:format("get_initial_state: ~w~n", [ResTyp_Pid, initial_state, [ResInst_Pid, TypeOptions]]),

...
loop() ->
	receive
		{initial_state, [ResInst_Pid, [Root_ConnectorPid, TypeOptions]], ReplyFn} ->
			C = discover_circuit(Root_ConnectorPid),
...
extract({C, _ , Iter }, ResLoop) ->
		ResPid = connector:get_ResInst(C),
...
discover_circuit([C | Todo_List], Circuit) ->
	{Updated_Todo_list, Updated_Circuit} =
...
process_connection(C, error, Todo_List, Circuit) ->
    ...
    CC = connector:get_connected(C),
	Updated_Todo_list = [ CC | Todo_List],
	ResPid = connector:get_ResInst(C),
	C_list = resource_instance:list_connectors(ResPid),
	{C_list ++  Updated_Todo_list, Updated_Circuit};
